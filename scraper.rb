...
class MembersPage < Scraped::HTML
  ...
  field :members do
    members_table.xpath('.//tr[td[2]]').map do |tr|
      ...
      data[:party] ||= party_lookup.fetch(data[:partyLabel])[:id]
      data
    end
  end
  ...
  def party_table
    # Need the second table to get the CPP
    noko.xpath('//table[.//td[contains(.,"Affiliation")]]')[1]
  end
...
class PartyRow < Scraped::HTML
  ...
  field :shortname do
    tds[0].css('a').map(&:text).map(&:tidy).last.gsub(')', '(').split('(').last
  end
...
class MemberRow < Scraped::HTML
  ...
  field :constituencyLabel do
    tds[0].css('a').map(&:text).map(&:tidy).first || tds[0].text.tidy
  end
...
url = 'https://en.wikipedia.org/wiki/List_of_MPs_elected_in_the_2012_Ghanaian_parliamentary_election'
